...
@Component("FC24")
public class Fc24ScoringEngine implements ScoringEngine {
    ...
    public Map<String, Integer> calculateScore(Map<String, Object> rawData, GameRule rule) {
        ...
        int goalsScoredPoint = ((Number) rule.getRuleConfig().getOrDefault("goalsScoredPoint", 1)).intValue();

        for (String teamId : rawData.keySet()) {
            Map<String, Object> teamData = (Map<String, Object>) rawData.get(teamId);
            int goals = ((Number) teamData.getOrDefault("goals", 0)).intValue();
            
            result.put(teamId, goals * goalsScoredPoint);
        }
